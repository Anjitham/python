...
# 2) print name of all movies
movie_name=[movi.get("title") for movi in movies]
print("The name of all movies:",movie_name)

# 3) print movie having genres>2 and count also
movie_gnr=[movi.get("title") for movi in movies if len(movi.get("genres"))>2]
# print(movie_gnr)
print("count of movies having genres >2 : ",len(movie_gnr))

# 4)print movie names and their count having bigraphy as genres
biography_movies=[movi.get("title") for movi in movies if "Biography" in movi.get("genres")] 
print(biography_movies)
print("count of movies having genre=biography : ",len(biography_movies))

# 5)print movie names whose director name starts with A
dir_name=[movi.get("director") for movi in movies if movi.get("director").startswith("A")]
print(dir_name)

# 6)print url length of 146th movie  &&&&
url_len=max(movies,key=lambda d: len(d.get("posterUrl")))
print("max longest url is:",url_len.get("id"))

# 7) released aftrer 2000
rlsyr_2k=[movi.get("title") for movi in movies if int(movi.get("year"))>2000]
print("name of movies released after 2000",rlsyr_2k)

# 8)run time < 90 and >150
runtime_name=[movi.get("title") for movi in movies if int(movi.get("runtime"))<90 or int(movi.get("runtime"))>150]
print("runtime between 90 and 150 : ",runtime_name) 

# 9)same director
dir_list=[movi.get("director") for movi in movies ]
same_dir=max(dir_list)
print("repeated director name:",same_dir)


#  10) movie having  longest runtime
long_run_movie=max(movies,key=lambda d: int(d.get("runtime")))
print("highest run time movie is : ",long_run_movie.get("title"))
...
for movi in movies:
    ...
    if year in year_cnt:
            ...
            year_cnt[year]=1
# print(year_cnt)
key_value_pairs=[(v,k) for k,v in year_cnt.items()]
print("max movie realeased in the year of:",max(key_value_pairs))

# # 11) print movie name corresponding to given id
# film_id=int(input("Enter an id to search:"))
# id_fetch=[movi.get("title") for movi in movies if film_id==movi.get("id")]
# print("name of the movie corresponding is:",id_fetch)


# 12)fetch genre=comedy and drama
sciFi_comedy_movie=[movi.get("title") for movi in movies if "Sci-Fi" in movi.get("genres") and "Comedy" in movi.get("genres")]
print("movie genres SciFi and comedy : ",sciFi_comedy_movie)

# genre wise max
genre_cnt={}
for movi in movies:
       genre=movi.get("genres")
       for gn in genre:
             if gn in genre_cnt:
                   genre_cnt[gn]+=1
             else:
                    genre_cnt[gn]=1
print(genre_cnt)
max_genre=[(v,k) for k,v in genre_cnt.items()]
print("Max repeated genre is : ",max(max_genre))
